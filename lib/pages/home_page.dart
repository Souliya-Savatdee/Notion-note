  ...
  Future<void> fetchNotes() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        int lastId = data.isNotEmpty
            ? data
                .map<int>((noteJson) => int.tryParse(noteJson['id']) ?? 0)
                .reduce((value, element) => value > element ? value : element)
            : 0;

        List<Note> notes = data
            .where((noteJson) => noteJson['trash'] != "TRUE")
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          Navigator.push(
            ...
            MaterialPageRoute(
                builder: (context) =>
                    AddCardNote(lastId: _lastId, fetchDataCallback: fetchNotes)),
          );
        },
        tooltip: "Add notes",
        backgroundColor: bgColor,
